...
function initializeConstants() {
    chrome.storage.sync.get(['HIGH_CONFIDENCE_THRESHOLD', 'LOW_CONFIDENCE_THRESHOLD'], function(items) {
        HIGH_CONFIDENCE_THRESHOLD = items.HIGH_CONFIDENCE_THRESHOLD !== undefined ? items.HIGH_CONFIDENCE_THRESHOLD : 0.5;
        LOW_CONFIDENCE_THRESHOLD = items.LOW_CONFIDENCE_THRESHOLD !== undefined ? items.LOW_CONFIDENCE_THRESHOLD : 0.1;
        // Use the constants as needed
        console.log(HIGH_CONFIDENCE_THRESHOLD, LOW_CONFIDENCE_THRESHOLD);
    });
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  switch (message.action) {
    case 'RESET_ALL_IMAGES':
      removeTextElements();
      break;
    case 'SELECT_FIRST_N_IMAGES':
      const images = document.querySelectorAll('img[class="YQ4gaf"]');
      const top10Images = Array.from(images).slice(0, 10);
      for (const image of top10Images) {
        chrome.runtime.sendMessage({ action: 'SELECTED_FIRST_N_IMAGES_FROM_CONTENT', src: image.src });
      }
      break
    case 'IMAGE_CLICKED':
